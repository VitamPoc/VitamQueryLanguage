...
public class CbQueryParserTest {
    private static final String exampleBothEsCb = 
            "{ $query : [ { $path : [ 'id1', 'id2'] },"+
    ...
    private static final String exampleCb = 
            "{ $query : [ { $path : [ 'id1', 'id2'] },"+
                "{ $and : [ {$exists : 'mavar1'}, {$missing : 'mavar2'}, {$isNull : 'mavar3'}, { $or : [ {$in : { 'mavar4' : [1, 2, 'maval1'] }}, { $nin : { 'mavar5' : ['maval2', true] } } ] } ] },"+
    ...
    public void testParse() {
        try {
            CbQueryParser command1 = new CbQueryParser(true);
            command1.parse(exampleBothEsCb);
            fail("Should refuse the request since ES is not allowed");
        } catch (Exception e) {
        }
        try {
            CbQueryParser command1 = new CbQueryParser(true);
            command1.parse(exampleCb);
            ...
            /*query.addRequests(andRequest().addToBooleanRequest(termRequest("mavar16", "motMajuscule").addTermRequest("mavar17","simplemot"),
            ...
            CbQueryParser command = new CbQueryParser(true);
